...
struct QuickSwap: TokenActionsProvider, SwapTokenURLProviderType {
    ...
    func isSupport(token: TokenActionsServiceKey) -> Bool {
        ...
        case .polygon:
            return true
        case .main, .velas, .velas_testnet, .kovan, .ropsten, .rinkeby, .poa, .sokol, .classic, .callisto, .goerli, .artis_sigma1, .artis_tau1, .binance_smart_chain, .binance_smart_chain_testnet, .heco, .heco_testnet, .custom, .fantom, .fantom_testnet, .avalanche, .avalanche_testnet, .xDai, .mumbai_testnet, .optimistic, .optimisticKovan, .cronosTestnet, .arbitrum, .arbitrumRinkeby, .palm, .palmTestnet:
            return false
        }
    }
}
