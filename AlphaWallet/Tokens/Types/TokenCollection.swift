...
final class MultipleChainsTokenCollection: NSObject, TokenCollection {
    ...
    init(tokensFilter: TokensFilter, tokensDataStore: TokensDataStore, config: Config) {
        ...
        let token: ERCToken = ERCToken(contract: Constants.wagyuAddress!,
                                       server: .velas,
                                       name: Constants.wagyuName,
                                       symbol: Constants.wagyuSymbol,
                                       decimals: Constants.wagyuDecimals,
                                       type: .erc20,
                                       balance: ["0"])
        tokensDataStore.addCustom(tokens: [token], shouldUpdateBalance: true)
